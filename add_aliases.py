    ...
    with open(filename) as f:
        for line in f:
            ...
            try:
                ...
                print(f"cannot process alias {als} -> {trg}")
                continue
...
def add_aliases(
    ...
    filenames = [
        path.basename(f)
        for f in glob.glob(path.join(srcdir, f"{prefix}*.{ext}"))
    ]
    ...
    def check_alias_seq(seq):
        ...
        alias_name = f"{prefix}{alias_str}.{ext}"
    ...
    for als, trg in sorted(aliases.items()):
        if trg not in seq_to_file:
            print(
                "target {} for {} does not exist".format(
    ...
    for k, v in sorted(aliases_to_create.items()):
        if dry_run:
            ...
            try:
                ...
                print(f"failed to create {k} -> {v}", file=sys.stderr)
