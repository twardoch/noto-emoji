...
def get_seq_to_file(image_dir, prefix, suffix):
    ...
    for name in os.listdir(image_dir):
        ...
        if seq in seq_to_file:
            raise Exception(f'duplicate sequence for "{name}" in {image_dir}')
...
def cp_name(cp):
    ...
    return "{}{:04X}".format("u" if cp > 0xFFFF else "uni", cp)
...
def update_ttx(in_file, out_file, image_dirs, prefix, ext, aliases_file):
    ...
    if not seq_to_file:
        raise ValueError(
            'no sequences with prefix "{}" and extension "{}" in {}'.format(
