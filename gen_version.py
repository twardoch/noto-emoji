...
def _get_existing_version(lines):
    ...
    def check_version(new_version):
        if version is not None and new_version != version:
            raise Exception(
                "version {} and namerecord version {} do not match".format(
                    version, new_version
                )
            )
        return new_version
...
def _version_compare(lhs, rhs):
    ...
    if lmaj != rmaj:
        return lmaj - rmaj
    if llen != rlen:
        raise Exception(f'minor version lengths differ: "{lhs}" and "{rhs}"')
    return lmin - rmin
...
def _get_repo_version_str(beta):
    ...
    if beta is not None:
        ...
        return f"GOOG;noto-emoji:{date_str};BETA {beta}"
    ...
    ymd = "".join(m.groups())
    return f"GOOG;noto-emoji:{ymd}:{commit[:12]}"
...
def update_version(srcfile, dstfile, version, beta):
    ...
    if not version:
        ...
        raise Exception(
            f"new version {version} is <= current version {current_version}"
        )
    version_str = f"Version {version};{_get_repo_version_str(beta)}"
