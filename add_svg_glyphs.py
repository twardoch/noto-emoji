...
from fontTools.ttLib.tables import _g_l_y_f, otTables
...
def add_image_glyphs(in_file, out_file, pairs):
    ...
    for glyphstr, filename in pairs:
        logging.debug(
            "Adding glyph for U+%s", ",".join(["%04X" % ord(char) for char in glyphstr])
        )
...
def main(argv):
  ...
  GSUB lookup (if existing) are modified."""
    ...
    parser.add_argument("in_file", help="Input ttx file name.", metavar="fname")
    parser.add_argument("out_file", help="Output ttx file name.", metavar="fname")
    parser.add_argument(
        "image_prefix", help="Location and prefix of image files.", metavar="path"
    )
    parser.add_argument(
        "-i",
        "--include",
    ...
    parser.add_argument("-l", "--loglevel", help="log level name", default="warning")
