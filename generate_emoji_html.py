...
def _generate_row_cells(key, font, aliases, excluded, dir_infos, basepaths, colors):
    ...
    def _text_cell(text_dir):
        ...
        return f'<span class="efont" dir="{text_dir}">{text}</span>'
...
def _get_image_data(image_dir, ext, prefix):
    ...
    fails = []
    result = {}
    expect_re = re.compile(fr"{prefix}([0-9A-Fa-f_]+).{ext}")
    for f in sorted(glob.glob(path.join(image_dir, "*.%s" % ext))):
        ...
        if not m:
            if filename.startswith("unknown_flag.") or filename.startswith("p4p_"):
                continue
            fails.append(f'"{expect_re.pattern}" did not match: "{filename}"')
            continue
        seq = m.group(1)
        this_failed = False
        try:
...
def _add_aliases(keys, aliases):
    for k, v in sorted(aliases.items()):
        ...
        if k in keys:
            ...
            print(f"have alias image {k_str}, should use {v_str}{msg}")
        elif v not in keys:
            print(f"can't use alias {k_str}, no image matching {v_str}")
...
def _generate_info_text(args):
    ...
    lines.append(
        f"generated by {path.basename(__file__)} on {datetime.datetime.now()}"
    )
    return "\n  ".join(lines)
...
def _instantiate_template(template, arg_dict):
    ...
    ids = {m.group(1) for m in id_regex.finditer(template)}
