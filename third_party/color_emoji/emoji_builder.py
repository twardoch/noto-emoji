...
class CBDT:
    def __init__(self, font_metrics, options=(), stream=None):
        self.stream = stream if stream is not None else bytearray()
    ...
    def write_glyphMetrics(self, width, height, big_metrics):
        ...
        y_bearing = int(round(line_ascent - 0.5 * (line_height - height)))
        ...
        try:
            if big_metrics:
                ...
                self.write(
                    struct.pack("BBbbB", height, width, x_bearing, y_bearing, advance)
                )
        except Exception as e:
            raise ValueError(
                "%s, h: %d w: %d x: %d y: %d %d a:"
    ...
    def write_format1(self, png):
        ...
        if img.get_format() != cairo.FORMAT_ARGB32:
            raise Exception(
                "Expected FORMAT_ARGB32, but image has format %d" % img.get_format()
            )
    ...
    def image_write_func(self, image_format):
        if image_format == 1:
            return self.write_format1
        if image_format == 17:
            return self.write_format17
        if image_format == 18:
            return self.write_format18
        return None
...
class CBLC:
    ...
    def write_indexSubTable1(self, glyph_maps):
        ...
        for gmap in glyph_maps[:-1]:
            self.write(
                struct.pack(">L", gmap.offset - imageDataOffset)
            )  # ULONG offsetArray
    ...
    def write_bitmapSizeTable(self, glyph_maps):
        ...
        for gmap in glyph_maps[1:-1]:
            if last_glyph + 1 != gmap.glyph or last_image_format != gmap.image_format:
                headers.extend(
                    struct.pack(">HHL", start, last_glyph, headersLen + len(subtables))
                )
...
""",
    ...
    font_metrics = FontMetrics(
        font["head"].unitsPerEm, font["hhea"].ascent, -font["hhea"].descent
    )
    print(
        "Font metrics: upem=%d ascent=%d descent=%d."
    ...
    image_format = (
        ...
        else (17 if "small_glyph_metrics" in options else 18)
    )
    ...
    for img_prefix in img_prefixes:
        ...
        for uchars, img_file in img_files.items():
            ...
            if "verbose" in options:
                ",".join(["%04X" % ord(char) for char in uchars])
