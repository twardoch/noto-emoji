...
class _Elem_Node:
    ...
    def __repr__(self):
        ...
        if self.contents:
            line.append(f" contents[{len(self.contents)}]: '{self.contents}'")
        line.append(")")
        return "".join(line)
        ...
        def _clean_elem(self, node):
            ...
            for k, v in node.attrs.items():
                if node.name == "svg" and k in [
                    ...
                    if k == "viewBox":
                        ...
                        logging.debug(f'keep "{k}" = "{v}"')
                        nattrs[k] = v
                    logging.debug(f"removing {k}={v}")
                    continue
                v = re.sub(r"\s+", " ", v)
                nattrs[k] = v
            ...
            if node.name == "svg":
                ...
                if viewBox and (int(x) != 0 or int(y) != 0):
                    logging.warn(f'viewbox "{viewBox}" x: {x} y: {y}')
                    nattrs["viewBox"] = viewBox
            node.attrs = nattrs

            # if display:none, skip this and its children
            style = node.attrs.get("style")
            if (style and "display:none" in style) or node.attrs.get(
                "display"
            ) == "none":
                node.contents = []
                return
    ...
    class _Writer:
        ...
        def _write_node(self, node, lines, indent):
            ...
            if isinstance(node, _Text_Node):
                ...
                for k in ks:
                    v = node.attrs[k]
                    line.append(f" {k}={saxutils.quoteattr(v)}")
                if node.contents:
                    line.append(">")
...
def main():
    parser = argparse.ArgumentParser(description="Generate 'cleaned' svg files.")
    parser.add_argument("in_dir", help="Input directory.", metavar="dir")
    parser.add_argument(
        "-o",
        "--out_dir",
        help="Output directory, defaults to sibling of in_dir",
        metavar="dir",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-r",
        "--regex",
        help="Regex to select files, default matches all files.",
        metavar="regex",
        default=None,
    )
    parser.add_argument(
